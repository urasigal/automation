...
public class BroadcasterUDPOutputStreamCreationTest extends BaseTestZixiMainComponents{
	...
	"dec_type", "cols", "max_payload", "mtu_time", "rist", "testid"})
	@Test
	...
	String dec_type, String cols, @Optional("0") String max_payload, @Optional("0") String mtu_time, @Optional("0") String rist,  String testid) throws Exception {
		
		testFlowDescriptor.append("\nStarting the BroadcasterUDPOutputStreamCreationTest");
		...
		"remux_buff", "local_ip", "remux_restampdts", "uiport", "remux_pcr", "dec_type", "cols", "max_payload", "mtu_time", "rist", "testid"}, 
		new String[] {userName, userPass, login_ip, port, stream, streamname, host, id, rtp, fec, smoothing, ttl, remux_bitrate, df, local_port, 
		dec_key, type, rows, remux_buff, local_ip, remux_restampdts, uiport, remux_pcr, dec_type, cols, max_payload, mtu_time, rist, testid });
		...
		ttl, remux_bitrate, df, local_port, dec_key, type, rows, remux_buff, local_ip, remux_restampdts, uiport, remux_pcr, dec_type, cols, max_payload, mtu_time, rist);
